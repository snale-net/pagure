...
dependencies["$group-$index"]="$mpi_dep python/$compilo/${pythonVersion} python-modules/$compilo/${pythonVersion} cmake/$compilo/3.20.5 swig/$compilo/4.0.1 lapack-blas/$compilo/3.9.1 vtk/$compilo/7.1.1 boost/$mpilib/$pythonlib/$compilo/1.72.0 petsc/$mpilib/$pythonlib/$compilo/tferma-v1.0 eigen/$mpilib/$compilo/3.2.3"
dirinstall["$group-$index"]="${name["$group-$index"]}/$compilo/${version["$group-$index"]}"
args["$group-$index"]=""
dirmodule["$group-$index"]="${name["$group-$index"]}/$compilo"
modulefile["$group-$index"]="#%Module1.0
