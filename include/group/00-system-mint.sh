...
sudo apt-get install libfreetype6-dev libjpeg-dev libpng-dev libx11-dev libxaw7-dev m4 patch make cmake autoconf bison flex libcurl4-gnutls-dev liblzma-dev libbz2-dev 2>&1 >&3 | tee -a $LOGFILE && leave

if [ "$pythonInterpreter" != "none" ]; then # only-if-Python
	if (( $(echo "$pythonVersion >= 3.0" | bc -l) )); then # only Python>=3.0
		# Python v3.x
		sudo apt-get install python3-dev libffi-dev 2>&1 >&3 | tee -a $LOGFILE && leave
		...
		# Python v2.x
		sudo apt-get install python-dev libffi-dev 2>&1 >&3 | tee -a $LOGFILE && leave
		# GDAL deps
