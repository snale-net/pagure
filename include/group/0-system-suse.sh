...
sudo $zypperCmd freetype-devel freetype2-devel libpng-devel libX11-devel libXrender-devel libXaw-devel libXfixes-devel freeglut-devel Mesa-libEGL-devel m4 patch make cmake libexpat-devel autoconf bison flex libcurl-devel xz-devel 2>&1 >&3 | tee -a $LOGFILE && leave
sudo $zypperCmd libopenssl-devel 2>&1 >&3 | tee -a $LOGFILE && leave
sudo $zypperCmd doxygen 2>&1 >&3 | tee -a $LOGFILE && leave
