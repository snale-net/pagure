...
sudo apt-get install $aptArgs libfreetype6-dev libjpeg-dev libpng-dev libx11-dev libxaw7-dev m4 patch make cmake autoconf bison flex libcurl4-gnutls-dev liblzma-dev libbz2-dev 2>&1 >&3 | tee -a $LOGFILE && leave
...
# Web deps
sudo apt-get $aptArgs install libarchive-tools nfs-kernel-server php 2>&1 >&3 | tee -a $LOGFILE && leave
