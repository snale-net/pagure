  ...
  then	
	  ...
	  for ((index=1;index<=$maxFile;index++)) do 
		  ...
		  then		
			...
			then
				if [[  "$compiler" == "intel" ]] ; then					
                    CC="icc -std=c99" $pythonInterpreter setup.py config --compiler=intelem --fcompiler=intelem build_clib --compiler=intelem --fcompiler=intelem build_ext --compiler=intelem --fcompiler=intelem install --user || leave 1 
				else
					$pythonInterpreter setup.py install --user || leave 1
				fi
			fi
