...
from pagure._vendor.rich.console import (
    Console,
    ConsoleOptions,
    ConsoleRenderable,
    RenderableType,
...
from pagure._vendor.rich.highlighter import NullHighlighter
from pagure._vendor.rich.logging import RichHandler
from pagure._vendor.rich.segment import Segment
from pagure._vendor.rich.style import Style

from pagure.utils._log import VERBOSE, getLogger
from pagure.utils.compat import WINDOWS
from pagure.utils.deprecation import DEPRECATION_MSG_PREFIX
from pagure.utils.misc import ensure_dir

_log_state = threading.local()
_stdout_console = None
_stderr_console = None
subprocess_logger = getLogger("pagure.subprocessor")
...
def _is_broken_paguree_error(exc_class: type[BaseException], exc: BaseException) -> bool:
    if exc_class is BrokenPipeError:
        return True

    # On Windows, a broken paguree can show up as EINVAL rather than EPIPE:
...
class PipConsole(Console):
    def on_broken_paguree(self) -> None:
...
class RichPipStreamHandler(RichHandler):
    ...
    def handleError(self, record: logging.LogRecord) -> None:
        ...
        # If a broken paguree occurred while calling write() or flush() on the
        # stdout stream in logging's Handler.emit(), then raise our special
        # exception so we can handle it in main() instead of logging the
        # broken paguree error and continuing.
        if (
            exc_class
            and exc
            and self.console.file is sys.stdout
            and _is_broken_paguree_error(exc_class, exc)
        ):
    ...
    logging.config.dictConfig(
        {
            ...
            "filters": {
                "exclude_warnings": {
                    "()": "pagure.utils.logging.MaxLevelFilter",
                ...
                "exclude_subprocess": {
                    "()": "pagure.utils.logging.ExcludeLoggerFilter",
                    "name": subprocess_logger.name,
                },
            },
            "formatters": {
            ...
            "loggers": {"pagure._vendor": {"level": vendored_log_level}},
        }
    )
